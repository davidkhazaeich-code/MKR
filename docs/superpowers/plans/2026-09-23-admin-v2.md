...
- Produces (Icon.tsx) : `export type IconName` (exporté) ; ajoute `log-in, users, list, calendar-days, handshake, download, chevron-left, chevron-right, chevron-down, more-horizontal, sun, moon, monitor, sliders, external-link, refresh, send, receipt, flag, user, copy, video, bell, home, info` ; `whatsapp` devient le vrai logo (chemin plein, `fill="currentColor"`).
...
- Connexion : chrome sombre, logo, champ caché `username=admin` (`autocomplete="username"`, visuellement masqué), champ jeton `autocomplete="current-password"`, `next` par défaut `/admin`, message d'erreur, bouton primaire « Se connecter » (icône `log-in`), mention « Connexion mémorisée 30 jours sur cet appareil ».
...
- Agenda « Visios » : groupes `AgendaGroup`, ligne = heure (ou date + heure), nom + `EN` si anglais, discipline · session, boutons WhatsApp (icône seule) et « Réservation » (lien secondaire vers `https://cal.com/booking/{visio_booking_uid}` si uid, nouvel onglet).
