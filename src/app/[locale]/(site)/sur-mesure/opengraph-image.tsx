import { createOgImageResponse, OG_SIZE, OG_CONTENT_TYPE } from '@/lib/og-template'

export const runtime = 'nodejs'

const COPY = {
  fr: {
    alt: 'Camp Sur Mesure · Solo, Duo, Trio ou Quatuor',
    keywords: 'Sur Mesure · Solo · Duo · Trio',
    title: 'Tes dates au Caucase',
    subtitle: 'Solo à Quatuor. Lutte, MMA ou combo Daghestan + Tchétchénie sur devis.',
  },
  en: {
    alt: 'Custom Camp · Solo, Duo, Trio or Quartet',
    keywords: 'Custom · Solo · Duo · Trio',
    title: 'Your dates in the Caucasus',
    subtitle: 'Solo to Quartet. Wrestling, MMA or Dagestan and Chechnya combo on quote.',
  },
} as const

export async function generateImageMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: raw } = await params
  const locale = raw === 'en' ? 'en' : 'fr'
  return [{ id: 'og', alt: COPY[locale].alt, size: OG_SIZE, contentType: OG_CONTENT_TYPE }]
}

export default async function OG({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: raw } = await params
  const locale = raw === 'en' ? 'en' : 'fr'
  const c = COPY[locale]
  return createOgImageResponse({
    keywords: c.keywords,
    title: c.title,
    subtitle: c.subtitle,
    bgImage: '/og-bg/sparring-mma-wall.png',
    locale,
  })
}
